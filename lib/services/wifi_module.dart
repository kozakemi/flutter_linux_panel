...
class WiFiModule extends WebSocketBaseModule {
  ...
  Future<WebSocketResponse> onHandleMessage(WebSocketRequest request) async {
    ...
    // 返回一个默认的成功响应，类型需将 *_request 映射为 *_response
    final responseType = request.type.replaceFirst(RegExp(r'_request$'), '_response');
  ...
  void _handleStatusChangedEvent(WebSocketEvent event) {
    try {
      ...
      developer.log('WiFi状态变化: ${status.statusDescription}', name: 'WiFiModule');
    } catch (e) {
  ...
  void _handleScanCompletedEvent(WebSocketEvent event) {
    try {
      ...
      developer.log('WiFi扫描完成，发现${scanResult.networks.length}个网络', name: 'WiFiModule');
    } catch (e) {
